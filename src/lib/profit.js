...
export default async function createProfitFunc(
  BASE_USD_BUDGET = 50,
  FEE = "0.000075"
) {
    ...
    for (let i = 0; i < stables.length; i += 1) {
      ...
      if (marketsData[p1Stable] && markets[p1Stable]) {
        ...
        return new Decimal(BASE_USD_BUDGET)
          ...
          .toDecimalPlaces(
            marketsData[symbol].precision.amount,
            Decimal.ROUND_DOWN
          );
      }
      if (marketsData[stablep2] && markets[stablep2]) {
        ...
        return new Decimal(BASE_USD_BUDGET)
          ...
          .toDecimalPlaces(
            marketsData[symbol].precision.quote,
            Decimal.ROUND_DOWN
          );
      }
    }
    ...
    for (let j = 0; j < pairsToTest.length; j += 1) {
      ...
      for (let i = 0; i < 3; i += 1) {
        ...
        if (markets[p12] && marketsData[p12]) {
          ...
          const bidPrice = Decimal(markets[p12].bidPrice).toDecimalPlaces(
            marketsData[p12].precision.price,
            Decimal.ROUND_DOWN
          );
          budget = budget.toDecimalPlaces(
            marketsData[p12].precision.amount,
            Decimal.ROUND_DOWN
          );
          ...
          orders.push({
            ...
            amount: budget,
            price: bidPrice,
            link: `https://www.binance.com/en/trade/${p12.replace(
              "/",
              "_"
            )}?layout=pro`,
            total: budget
              .mul(bidPrice)
              .toDecimalPlaces(
                marketsData[p12].precision.quote,
                Decimal.ROUND_DOWN
              ),
          });
          ...
          const askPrice = Decimal(markets[p21].askPrice).toDecimalPlaces(
            marketsData[p21].precision.price,
            Decimal.ROUND_DOWN
          );
          ...
          budget = budget
            .div(askPrice)
            .toDecimalPlaces(
              marketsData[p21].precision.amount,
              Decimal.ROUND_DOWN
            );
          ...
          orders.push({
            ...
            amount: budget,
            price: askPrice,
            link: `https://www.binance.com/en/trade/${p21.replace(
              "/",
              "_"
            )}?layout=pro`,
            total: prevBudget.toDecimalPlaces(
              marketsData[p21].precision.quote,
              Decimal.ROUND_DOWN
            ),
          });
        } else {
